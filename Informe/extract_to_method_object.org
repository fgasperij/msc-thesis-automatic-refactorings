...
interfaz de usuario busca simplificar la tarea y guiar su utilización. Al
ser un refactoring de alto nivel es esperable que el usuario no pueda visualizar
...
construir esta imagen y poder entender rápidamente todas las configuraciones que
se le presentan.

:REFERENCE:
a todo lo que sea de smalltalk y demasiado básico para explicar aquí mismo.
:END:
La interfaz se puede acceder a través de la lista de mensajes de la ~BrowserWindow~:
...
:IMAGE:
Ejemplo del formulario con Object>>executeMethod
:END:
