...
Baker>>breadWith: someExtraIngredients
  dangerousIngredientsPresent := someExtraIngredients anySatisfy: [:anIngredient |
    anIngredient isSpicy or: [anIngredient isRadioactive]].
  dangerousIngredientsPresent ifTrue: [BakingError signal: 'Dangerous ingredient present.'].
...
- Todas las variables a parametrizar deben contar con una 
...
Todas las clases responden el mensaje ~compile:~, por lo cual para poder agregar
...
#+BEGIN_SRC
ExampleClass>>k1: aP1 k2: aP2
  ^self delegateWith: p1 and: p2.
#+END_SRC

este método tiene las variables a parametrizar ~aP1~, ~aP2~ y ~self~. Para cada una de ellas
