...
~BrowserWindow~, o una en la lista de una ~MessageSetWindow~:
...
~ExtractToMethodObjectApplier~. ~ETMOApplier~ es subclase de
~RefactoringApplier~. ~RefactoringApplier~ encapsula el flujo estándar de
aplicación de un refactoring y el control de errores común a todos los
refactorings existentes:
...
~ETMOApplier~ respeta el mismo flujo casi por completo pero sobreescribe ~value~
porque le realiza una pequeña modificación. Al solicitar del usuario varios
parámetros, a diferencia de otros refactorings más simples que suelen requerir
solo 1 o 2, resultaría inconveniente que ante un error el flujo sea abortado,
que es lo que sucede en el resto de los refactorings. Sería inconveniente porque
el usuario debería especificar nuevamente todos los parámetros. Por esta razón,
~ETMOApplier~ no aborta el flujo en caso de error, le muestra un mensaje
explicando la causa del error al usuario:
...
~ETMOApplier~ se encarga de crear una instancia del formulario y luego, como ya
vimos que encapsulan todos los appliers, crear el refactoring y
aplicarlo. ~ETMOForm~, subclase de ~SystemWindow~, se encarga de:

1. Presentarle el formulario al usuario abriéndose en el ~world~.

2. Crear una instancia de ~ETMORequest~ y utilizarla como contenedor de todos
   los modelos de ~Morph~ necesarios.

3. Notificarle a ~ETMOApplier~ de los eventos /cancel/, /close/  y /submit/.
...
también fueron reutilizados en el formulario del ~IntroduceNullObject~. Éstos
son:
...
usuario. ~ETMORequest~ se encarga de crear todos estos modelos y contenerlos
colaborando principalmente con el ~ETMOForm~, que se encarga de crear los
~Morph~ s correspondientes a ellos. Además, provee el acceso a el contenido de
estos modelos de una forma simple para que el ~ETMOApplier~ pueda crear el
refactoring sin necesidad de conocer sobre modelos.

** Aclaraciones adicionales

:WRITE:
Este próximo párrafo dónde tendría que estar?
:END:
Además, las validaciones que no dependen del input del usuario son realizadas
también antes de presentarle el formulario al usuario, de esta forma si el
método contiene una referencia a ~super~ el usuario se entera inmediatamente y
no después de llenar todos los parámetros necesarios.

:WRITE:
Sacar esta aclaración si se detalla nada.
:END:
Al finalizar el refactoring si detectamos algo que probablemente deba ser
modificado pero no es realizado automáticamente se le informa al usuario para
que sepa las limitaciones del refactoring que aplicó.
