...
:DIAGRAM:
Un diagrama de secuencia mostrando cómo intereactúan INO y RINWPP
:END:

:WRITE:
Qué son las variables a parametrizar? Es lo mismo que en ETMO? Si es lo mismo
hacer un vínculo
:END:
...
- Los bloques no contienen asignaciones a variables que no son temporales del
  ...
  :REFERENCE:
  Link a ETMO la parte de las [[file:extract_to_method_object.org::*No%20contiene%20asignaciones%20a%20variables%20que%20no%20sean%20temporales][asignaciones]] 
  :END:
  priorizar la simplicidad de la primer versión. Más detalles en ETMO.
...
los bloques que reciben los Nil Selectors y los Boolean Selectors.
...
consiste en reemplazar cada uno de los chequeos por nil especificados por los
elementos de su colaborador interno ~replacementParameters~. Como cada uno de
los elementos es una instancia de ~ReplaceIfNilWithPolymorphismParameters~ no
solo especifican el chequeo por nil, sino que también el keyword y el nombre del
parámetro de cada variable a parametrizar para realizar el reemplazo. Los cambios
que incluye la aplicación para cada cheque son:
...
* Elementos que no estoy viendo descriptos

- la búsqueda de los chequeos de ifNil
